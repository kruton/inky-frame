...
if (NOT Python3_FOUND)
    message(FATAL_ERROR "Python not found. Please ensure it is installed and its path is added to CMAKE_PREFIX_PATH.")
endif()

message("dir2uf2/py_decl: Using Python ${Python3_EXECUTABLE}")
...
if (EXISTS "${PIMORONI_TOOLS_DIR}/py_decl/py_decl.py")
    ...
    add_custom_target("${MICROPY_TARGET}-verify" ALL
        COMMAND ${Python3_EXECUTABLE} "${PIMORONI_TOOLS_DIR}/py_decl/py_decl.py" --to-json --verify "${CMAKE_CURRENT_BINARY_DIR}/${MICROPY_TARGET}.uf2"
...
if (EXISTS "${PIMORONI_TOOLS_DIR}/dir2uf2/dir2uf2" AND EXISTS "${PIMORONI_UF2_MANIFEST}" AND EXISTS "${PIMORONI_UF2_DIR}")
    ...
    add_custom_target("${MICROPY_TARGET}-with-filesystem.uf2" ALL
        COMMAND ${Python3_EXECUTABLE} "${PIMORONI_TOOLS_DIR}/dir2uf2/dir2uf2" --fs-compact --sparse --append-to "${MICROPY_TARGET}.uf2" --manifest "${PIMORONI_UF2_MANIFEST}" --filename with-filesystem.uf2 "${PIMORONI_UF2_DIR}"
        WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
